  ...
  filenames.forEach(filename => {
    if (extension(filename) === '.html') {
      ...
      files.forEach(file => {
        ...
        const nodeModules = file.match(
          /(?:href|src)="(node_modules.*.[css|js])"/
        );
        if (nodeModules) {
          let vendor = [];
          ...
          if (
            vendors.findIndex(vendor => vendor.absolute === resolve(src)) === -1
          ) {
            vendors.push(vendor);
            ...
            if (extension(src) === '.css') {
              const assets = fs
                .readFileSync(resolve(src), 'ascii')
                .toString()
                .match(/(?:url)\((.*?)\)/gi);
              if (assets) {
...
const copyFiles = (files, dest) => {
  files.forEach(file => {
    ...
    mkdirp.sync(dir);
    fs.createReadStream(file.absolute).pipe(
      fs.createWriteStream(resolve(dir, basename(file.src)))
    );

    if (fs.existsSync(`${file.absolute}.map`)) {
      fs.createReadStream(`${file.absolute}.map`).pipe(
        fs.createWriteStream(resolve(dir, `${basename(file.src)}.map`))
      );
    }
  });
};
...
const replaceRecursively = (filename, vendor) => {
  ...
  const replacement = `vendors/${vendor.name}/${vendor.filetype}/${basename(
    vendor.src
  )}`;
